...
func UrlRoutes(router fiber.Router) {
	router.Post("/api/new_order/:org_longitude/:org_latitude/:org_zipcode/:org_ext_num/:org_int_num/:dst_longitude/:dst_latitude/:dst_zipcode/:dst_ext_num/:dst_int_num/:size/:weight/:amount", func(c *fiber.Ctx) error {
		m := c.Queries()
		...
		if !ValidateSizeAndStatus(size, size_list) {
		...
		_, err = collection.InsertOne(ctx, new_pkg)
		if err != nil {
	...
	router.Post("/api/chage_status/:order_id/:new_status/:reembolso", func(c *fiber.Ctx) error {
		...
		if !ValidateSizeAndStatus(new_status, status_list) {
	...
	router.Get("/api/CheckPackage/:order_id", func(c *fiber.Ctx) error {
		var result models.Packages
		order_id := c.Params("order_id")
		filter := bson.M{"identifier": order_id}
